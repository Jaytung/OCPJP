 ...
 */
@WebServlet(urlPatterns = { "/member/credit_card_back.do" })
public class CreditCardCallBackServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			service.updateOrderStatusToPAID(member.getEmail(),orderId, card6no, card4no, auth_code, process_date, amount);// 呼叫前面步驟一完成的修改訂單狀態為已付款的程式
